...
for(set.name in names(dp.peaks.matrices)){
  ...
  for(chunk.name in names(optimal)){
    ...
    for(sample.id in names(optimal.list)){
      ...
      param.name <- as.character(optimal.df$peaks)
      ...
      min.err <- min(optimal.df$error)
      stopifnot(with(indices, optimal.df$error[start:end]) == min.err)
      one.interval <- 
    ...
    oracle.intervals[[set.name]][[chunk.name]] <-
...
save(oracle.intervals, file="oracle.intervals.RData")
