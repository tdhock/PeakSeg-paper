...
\begin{table*}
  \centering
  \begin{tabular}{lllll}
    name & complexity & smoothing & parameters to learn & learning algorithm \\
    \hline
    L1.reg & $s$ & 
    $e^\beta x_{i,1}^{w_1} \cdots x_{i, 41}^{w_{41}}$ & 
    $\beta, w_1, \dots, w_{41}\in\RR$ & interval regression \\
    log.bases.log.max & $s$ & 
    $e^\beta b_i^{w_1} (\max \mathbf y_i)^{w_{2}}$ & 
    $\beta, w_1, w_{2}\in\RR$ & interval regression \\
    s.constant \textbf{TODO} & $s$ & 
    $\beta$ & 
    $\beta\in\RR_+$ & grid search \\
    oracle.41 \textbf{TODO} & $s\left(1 + 4\sqrt{1.1 + \log(d_i/s)}\right)^2$ &
    $e^\beta x_{i,1}^{w_1} \cdots x_{i, 41}^{w_{41}}$ & 
    $\beta, w_1, \dots, w_{41}\in\RR$ & interval regression \\
    oracle.2 \textbf{TODO} & $s\left(1 + 4\sqrt{1.1 + \log(d_i/s)}\right)^2$ &
    $e^\beta b_i^{w_1} (\max \mathbf y_i)^{w_{2}}$ & 
    $\beta, w_1, w_{2}\in\RR$ & interval regression \\
    oracle.trained & $s\left(1 + 4\sqrt{1.1 + \log(d_i/s)}\right)^2$ &
    $\beta$ & $\beta\in\RR_+$ & grid search \\
    oracle.default & $s\left(1 + 4\sqrt{1.1 + \log(d_i/s)}\right)^2$ &
  ...
  \end{tabular}
  \caption{Maximum likelihood penalty functions considered in this paper.}
  \label{tab:penalties}
\end{table*}
...
TODO: figure showing different patterns in different data sets.
...
e^\beta (\log\max\mathbf y_i)^{w_1} (\log d_i)^{w_2}$. We trained the
...
TODO: discuss machine learning for recognizing different patterns,
versus using several different unsupervised algorithms for specific
data sets. E.g. car/face detector versus generic object detector
trained on various different data sets.
