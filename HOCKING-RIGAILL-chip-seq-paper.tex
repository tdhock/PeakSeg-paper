...
There are several differences between the \ref{PeakSeg} model we
propose and the previous Poisson segmentation model proposed by
\citet{Segmentor}. First, they proposed to search all possible
change-points to find the optimal segmentation with $s$ segments, but
we propose to constrain the possible change-points to the subset of
models that can be interpreted as peaks
(Section~\ref{sec:constrained}). Second, their pruned dynamic
programming algorithm (pDPA) is guaranteed to recover the exact
solution, whereas our proposed constrained dynamic programming
algorithm (cDPA) is a heuristic (Section~\ref{sec:dp-fails}). Also,
for segmenting a sequence of $d$ data points, the pDPA takes on
average $O(d\log d)$ time whereas our proposed cDPA takes $O(d^2)$
time. Finally, implementing our proposed cDPA takes only a small
modification to the standard DPA, which is much simpler to implement
than the pDPA.
