...
\begin{figure*}[t!]
  ...
  \caption{Test error of algorithms on seven annotated region data
    sets (each point shows one of six randomly selected
    train/test splits, and the shaded circle is the mean test
    error). Data set names show histone mark type (e.g. H3K36me3),
    annotator (AM), and cell types (immune). The multi-parameter
    supervised penalty learning methods had the lowest test error for
    all data sets.}
