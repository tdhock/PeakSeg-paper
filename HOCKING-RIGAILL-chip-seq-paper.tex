...
\sigma_{(t', t]}/(t-t')$ is the mean over the segment $(t', t]$. The optimal
Poisson loss (\ref{eq:rho}) for the is 
\begin{equation}
...
model with $s$ segments up to data point $t$. The key idea behind the
DPA is that any subsegmentation of an optimal segmentation
should be optimal. This can be formally written as a search for the best
change-point $t'$
\begin{equation}
...
In order to force the segmentation to obey the constraint of the
...
Note that we do not need to precompute the
$d(d-1)/2$ values of $c_{(t',t]}$ and $M_{(t', t]}$. That is because
the optimal cost $c_{(t',t]}$ and empirical mean $M_{(t', t]}$ are
simple functions of $\sigma_{(t', t]} =\sum_{j=t'+1}^t y_j$ which can be computed in
$\mathcal{O}(1)$ given the cumulative sum of the data-points,
$\sum_{j=1}^t y_j$. Hence the memory complexity of the cDPA is in
$\mathcal{O}(s_{\text{max}} d)$.
...
the constrained optimal segmentation $\mathbf{\tilde m}^3(\mathbf y)$ is
undefined, and the cDPA thus returns no model.
