...
$\lambda=\exp f(\mathbf x)=2$ and a model complexity of $h(s,
d)=s$. More recently, \citet{cleynen2013segmentation} applyied finite
sample model selection theory to obtain a more complicated model
complexity $h$ (Table~\ref{tab:penalties}) and a heuristic for
computing the penalty $\lambda$.
All of these penalty functions are completely unsupervised since they
ignore the annotated region labels in the training data set. 

These
models are named *.0 in Table~\ref{tab:penalties} and
