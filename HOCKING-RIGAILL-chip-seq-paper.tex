...
$\mathbf{\hat m}^1(\mathbf y), \dots, \mathbf{\hat
  m}^{s_{\text{\text{max}}}}(\mathbf y)$ can be computed in $O(s_{\text{\text{max}}}
d^2)$ time using dynamic programming (DP) algorithms \citep{bellman},
or in $O(s_{\text{\text{max}}} d \log d)$ time using pruned DP
...
model, and for choosing the number of segments $s$. We first review
the existing standard dynamic programming algorithm (DPA) for
segmentation, then propose a new constrained dynamic programming
...
$s\in\{1, \dots, s_{\text{\text{max}}}\}$ (Section~\ref{sec:supervised}).
...
number of segments $s$. These are based on theoretical
assumptions about the data (distribution, independence) which may or
may not be true in practice. For example, one can use asymptotic
arguments to get a penalty such as the AIC (Akaike73), BIC
\citep{Schwarz78}, or mBIC \citep{mBIC}. More recently,
\citet{cleynen2013segmentation} proposed an oracle penalty by applying
finite sample model selection theory to segmentation models of count
data. All of these penalty functions are completely unsupervised since
