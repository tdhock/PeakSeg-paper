  ...
  for(set.i in seq_along(train.sets)){
    ...
    for(validation.fold in 1:n.folds){
      ...
      for(tv in names(set.chunks)){
        ...
        for(chunk.name in chunk.names){
          ...
          sample.list <- oracle.optimal[[set.name]][[chunk.name]]
          optimal.dt <-
          ...
          names(dimnames(err.mat))[2] <- "peaks"
          err.dt <- data.table(melt(err.mat, value.name="errors"))
          join.dt <- inner_join(err.dt, overlap.dt, c("sample.id", "peaks"))
    ...
    for(chunk.name in set.chunks$test){
      ...
      for(model.name in names(fits)){
        ...
        unfiltered <- foverlaps(penalty.dt, optimal.dt, nomatch=0)
        ## data.table bug: need to filter results
        overlap.dt <- unfiltered %>%
        ...
        err.dt <- data.table(melt(err.mat, value.name="errors"))
        join.dt <-
          inner_join(err.dt, overlap.dt, c("sample.id", "peaks"))
        join.dt$regions <- regions[as.character(join.dt$sample.id)]
        stopifnot(nrow(join.dt) == nrow(overlap.dt))
...
save(oracle.regularized, file="oracle.regularized.RData")
